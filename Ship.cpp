...
//this->texture.loadFromFile("animated_asteroid.xcf");
 ...
 void Ship::update( float delta_time_seconds )
 {
  ...
  float sinRotation = std::sin( rotation * degree2radian );
   ...
   velocidadNave.x += velocidadAceleracion * delta_time_seconds * cosRotation;
   velocidadNave.y += velocidadAceleracion * delta_time_seconds * sinRotation;
//  }
  ...
  if( ship_speed > 0 ) //Si la nave se esta moviendo
  {
   ...
   //Aplico la resistencia en la direccion del movimiento
      velocidadNave.x -= fuerzaResistencia * delta_time_seconds * std::cos( angle_of_velocity );
       velocidadNave.y -= fuerzaResistencia * delta_time_seconds * std::sin( angle_of_velocity );
